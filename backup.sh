...
compressed_dir="/home/shanx/DevOps/ansible/apache/apache_backup"
...
function perform_backup {
	...
	do
		...
		tar --create --gzip --listed-incremental=$filename --file=$filename $backup_dir 2>> $LOG_LOC
		mv $filename $compressed_dir
		echo "Backing up is completed." >> $LOG_LOC

	done < $path
	echo; echo "Backup was completed at: " >> $LOG_LOC
	date >> $LOG_LOC
}
